...
class PackAssetsTest {
    ...
    fun `the typeface never supplies a codepoint shadr reserves`() {
        val root = generate()
        // A Nerd Font covers most of the private-use area, so a typeface that is allowed to
        // offer these codepoints shadows shadr's own art: shape fills become icons and a
        // `block` stops drawing a box. `skip` is what keeps the two apart.
    ...
    fun `both UI weights carry the private-use bitmaps`() {
        val root = generate()
        // Semibold once shipped the TTF alone, so any shape drawn in `font: shadr_semibold`
        // fell through to the typeface.
        for (weight in listOf(Glyphs.FONT_UI, Glyphs.FONT_UI_SEMIBOLD)) {
            val json = File(root, "assets/minecraft/font/$weight.json").readText()
            assertTrue(
