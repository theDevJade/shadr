...
class PaperBridge(
    ...
    /**
     * Must match whether the pack ships the frosted-glass post chain. Spectate the mob without
     * the chain and the player gets vanilla's colour inversion; ship the chain without
     * spectating and they get nothing. See [PaperCamera].
     */
    ...
    override fun pack(): ResourcePackService = ResourcePackService { player, url, sha1, forced ->
        // The id is the hash, so a client that already holds this build skips the download.
    ...
    /** Left click: the client reports it as an attack on the interaction stand-in. */
    ...
    /** Right click, and any stray world interaction while a page is open. */
    @EventHandler(priority = EventPriority.LOWEST)
...
/**
 * Each player keeps their own [LookMapper] so yaw accumulation survives across ticks; the
 * mapper is reset when a camera starts, otherwise the first sample would register as one
 * enormous mouse jerk.
 */
class PaperInput(private val plugin: Plugin, private val camera: PaperCamera) : InputSource {
    ...
    /** Called from the plugin's tick task. */
    fun tick() {
