...
/**
 * HUD parts are real Display entities, spawned non-persistent, hidden from every player but
 * their owner, and mounted on a carrier entity the owner rides. Without that carrier there is
 * nothing on screen. A display not attached to something inside the camera frustum gets culled
 * the moment the player looks away from where it nominally sits in the world.
 *
 * Updates diff against the previous frame. A hover changes one element out of a hundred, and
 * respawning the rest would flicker and cost a packet each.
 */
class PaperHudSink(private val plugin: Plugin) : HudSink {
    ...
    /**
     * Creates the carrier the HUD hangs from and seats it on the player. Vanilla re-sends the
     * passenger list on unrelated events, and Paper has no hook to suppress that, so
     * [ensureMounted] re-seats the carrier from the session tick.
     */
    ...
    /** Re-seat the carrier if vanilla passenger sync dropped it. Called each tick. */
    ...
    private fun spawn(owner: Player, hud: PlayerHud, draw: HudDraw): Display {
        ...
        // Visible to the owner only: a HUD is a per-player overlay.
        display.isVisibleByDefault = false
    ...
    private fun Display.applyDraw(draw: HudDraw) {
        ...
        transformation = Transformation(
            ...
            // `make_hud()` mirrors X, so without this flip glyphs render back-to-front.
            HUD_FACING,
    ...
    /** Rotation about Z, applied after the facing flip, for `rotationDeg` elements. */
    ...
    /**
     * `custom_model_data` drives the pack's `range_dispatch` to a corner-radius model, and
     * `dyed_color` feeds that model's dye tint.
     */
    ...
    private companion object {
        /**
         * Displays are mounted on the player, so range never really applies, but a small
         * view range would still let the client drop them during the mount's own update.
         */
