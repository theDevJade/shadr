 ...
 * Paper entry point.
 *
 * Lifecycle, in order, because each step depends on the last: read config -> build the
 * pack (pages can reference uploaded images, so the atlas is folded in first) -> host it
 * -> load pages -> start the tick loop.
 */
class ShadrPlugin : JavaPlugin() {
    ...
    /** Which vanilla programs are overridden. Written by the editor, read by the build. */
    ...
    /**
     * The lambda re-reads the registry on every call. A shader dropped in at runtime and picked
     * up by `/shadr reload` can then be spawned without restarting the server.
     */
    ...
    override fun onEnable() {
        ...
        // One source of truth for both halves of the frosted-glass effect. The pack ships the
        // post chain only when this override is on, and the camera spectates an EnderMan only
        // when it is on. Having one without the other looks worse than having neither.
        bridge = PaperBridge(
            this,
    ...
    private fun startEditor() {
        editor = runCatching {
            ...
            // A refused bind must not take the rest of the plugin down with it.
    ...
    /**
     * The running version comes from `plugin.yml`, which the build token-expands from the Gradle
     * project version. A version that will not parse becomes `Version.UNKNOWN` and loses to
     * every published release. An operator running a jar with a broken descriptor is told an
     * update exists, which is the better of the two failures.
     */
    ...
    /**
     * Matched by name, not identity. The session holds the page it was opened with, and the
     * edited one is a fresh value from the resolver.
     */
    ...
    /**
     * A shader reaches a client only inside a pack, so unlike a page edit this costs every
     * viewer a re-download.
     */
    ...
    /**
     * Safe to call at runtime: a rebuild changes the SHA-1, and joining players get the new
     * hash, so nothing has to be invalidated by hand.
     */
    fun reload() {
        rebuildPack()
    ...
    /**
     * The interval matters. A session re-renders the whole page when any placeholder moves, so
     * a one-tick refresh on a page with a clock re-sends every element twenty times a second and
     * starves the cursor.
     */
    ...
    private fun placeholderResolver(player: PlayerId): dev.shadr.core.page.PlaceholderResolver {
        // Built-ins first, PlaceholderAPI second. shadr's own names come from a local snapshot,
        // so only the names it does not recognise cost a PAPI call. In the other order,
        // `%shadr_online%` would go through an expansion lookup on every dynamic element.
    ...
    /**
     * Resolved once at enable, not per session, since the presence check walks the plugin
     * manager. [detectPlaceholderApi] fills it eagerly so the operator hears at boot whether the
     * integration is live; a lazy value would say nothing until the first page opened.
     */
    private var papiResolver: dev.shadr.core.page.PlaceholderResolver? = null

    /** Called from `onEnable`; `softdepend` guarantees PlaceholderAPI is already enabled. */
    private fun detectPlaceholderApi() {
        ...
        if (papiResolver != null) {
            ...
            // Running without PAPI is a supported configuration, so this is information
            // and not a problem.
            logger.info(
    ...
    private fun builtinResolver(player: PlayerId): dev.shadr.core.page.PlaceholderResolver =
        dev.shadr.core.page.BuiltinPlaceholders {
            ...
            // Minecraft's day starts at 06:00, hence the offset.
    ...
    /**
     * `/shadr shader <id> [scale] [colour] | list | clear [handle]`. Each placement gets its own
     * handle, so repeated spawns accumulate and do not replace each other.
     */
    ...
    /**
     * Every request gets a freshly minted, short-lived token. Echoing the server's own
     * long-lived secret into chat would leave a permanent credential in the server log and in
     * every chat-logging plugin, and revoking it would break the operator's own bookmark.
     */
    private fun editorCommand(sender: CommandSender, sub: String?): Boolean {
        ...
        // Re-issuing replaces the caller's previous link, so running this repeatedly does not
        // leave a trail of live credentials.
    ...
    /**
     * Seeds the data folder from what is bundled in the jar. Existing files are never
     * overwritten. These directories are where an operator keeps their own font and their own
     * pages, so this is first-run seeding and not a sync.
     */
    private fun unpackBundledAssets() {
        // A jar is not a directory, so there is no portable way to enumerate a resource folder
        // from inside one. The build generates the index from the same list `processResources`
        // copies from, so the two cannot drift.
    ...
    private companion object {
        /** Bind on every interface; the *advertised* host comes from config. */
        const val BIND_ALL = "0.0.0.0"

        const val EDITOR_PERMISSION = "shadr.editor"

        /** Placing a shader spawns a world entity, so it is a separate grant from /shadr. */
