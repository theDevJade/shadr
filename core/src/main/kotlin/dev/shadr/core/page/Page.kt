...
enum class ElementType(val id: String, val defaultGlyph: Char = Glyphs.BACKGROUND) {
    /**
     * A filled rectangle: the [Glyphs.BACKGROUND] quad scaled to `size`.
     */
    BLOCK("block"),

    /**
     * A block whose rounding is on.
     */
    BLOCK_ROUNDED("block_rounded"),

    /** An ellipse, and a circle whenever `width` and `height` match. */
    CIRCLE("circle", Glyphs.CIRCLE),

    /** A very fake gradient. */
    ...
    /**
     * A progress bar or slider track.
     */
    PROGRESS("progress", Glyphs.SLIDER),

    /**
     * SDF versions of the block, wow.
     */
    BLOCK_SDF("block_sdf"),
    ...
    /**
     * A custom fragment program drawn in place of an item.
     */
    SHADER("shader"),
...
data class EffectDef(
    ...
    /** Interpolation ticks the client should use for this effect (20 ticks per second). */
