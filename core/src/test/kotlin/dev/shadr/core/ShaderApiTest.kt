...
class ShaderApiTest {
    ...
    @Test
    fun `reusing a handle replaces rather than accumulating`() {
        val world = FakeWorld()
    ...
    @Test
    fun `a platform without world support says so instead of failing silently`() {
    ...
    @Test
    fun `a non-positive scale is refused rather than spawning something invisible`() {
        val world = FakeWorld()
    ...
    @Test
    fun `the registry is re-read rather than captured`() {
