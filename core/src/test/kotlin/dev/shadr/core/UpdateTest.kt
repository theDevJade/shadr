...
class UpdateTest {
    ...
    @Test
    fun `newest by version rather than by publication order`() {
        val status = withReleases(
            """
            [
    ...
    @Test
    fun `a release with no plugin jar is reported rather than treated as nothing`() {
        val status = withReleases(
    ...
    @Test
    fun `malformed json is a failure rather than an exception`() {
    ...
    @Test
    fun `an unrecognised channel falls back to auto rather than failing to boot`() {
    ...
    @Test
    fun `a blank repository falls back to the default rather than requesting one named empty`() {
