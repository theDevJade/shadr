...
If the server finds no build at that path it serves a short instructions page rather than a 404,
so a blank-looking editor usually means the Flutter build is missing rather than that something
crashed.
...
paths outside the package directory, so the copy is generated instead of referenced.
...
Sibling keys that affect editing rather than hosting: `editor.autosave.enabled`,
...
configured but fails to load stops the editor rather than falling back to plain HTTP, because an
...
`/shadr editor link` mints a short-lived token per invocation instead of echoing the configured
one, which would otherwise leave a permanent credential in the server log and in every
chat-logging plugin. `/shadr editor revoke` drops the minted ones.
...
another plugin's copy. See [`../SURVEY.md`](../SURVEY.md) §1.6.
...
rounding. This is the largest gap in the editor and is tracked in
[`../SURVEY.md`](../SURVEY.md) §3.2.
