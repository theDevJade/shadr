...
void main() {
  ...
  testWidgets('the arrows move the caret while a field has focus', (tester) async {
    final (:model, :transport, :field) = await pump(tester);
    await focusField(tester);
    field.selection = const TextSelection.collapsed(offset: 5);
  ...
  testWidgets('escape leaves the field and keeps the selection', (tester) async {
    final (:model, :transport, :field) = await pump(tester);
    await focusField(tester);
  ...
  testWidgets('a value field commits on escape',
      (tester) async {
    final (:model, :transport) = connectedModel();
